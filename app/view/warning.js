...
function get_purple(){
    ...
    if(PURPLE[COUNTY]<3){tag.innerHTML = "低量級"}
    else if(PURPLE[COUNTY]<6){tag.innerHTML = "中量級"}
    else if(PURPLE[COUNTY]<8){tag.innerHTML = "高量級"}
    else if(PURPLE[COUNTY]<11){tag.innerHTML = "過量級"}
    else if(PURPLE[COUNTY]>=11){tag.innerHTML = "危險級"}
    else{tag.innerHTML = "此行政區沒有觀測站"}
}
...
function create_area(){
    Object.keys(AREA[COUNTY]).map((item)=>{
        ...
        if(AREA[COUNTY][item][0].maxWarning==="高危險"){ i.classList.add("black")}
        else if(AREA[COUNTY][item][0].maxWarning==="危險"){ i.classList.add("red")}
        else if(AREA[COUNTY][item][0].maxWarning==="警戒"){ i.classList.add("orange")}
        else if(AREA[COUNTY][item][0].maxWarning==="注意"){ i.classList.add("yellow")}
        i.innerText = item
...
function create_chart(){
    ...
    const week = ["星期日","星期一","星期二","星期三","星期四","星期五","星期六","星期日","星期一","星期二","星期三"]
    const weekday = today.getDay()
    const labels4today = [week[weekday],week[weekday+1],week[weekday+2],week[weekday+3]] 
    const labels4tomorrow = [week[weekday+1],week[weekday+2],week[weekday+3],week[weekday+4]] 
    const labels5 = [week[weekday],week[weekday+1],week[weekday+2],week[weekday+3],week[weekday+4]] 
    let dataset={}
    if(TOWN){
        const hotArray = []
...
function fill_map(){
    ...
    taiwan.forEach((e)=>{
        ...
        if(AREA[name][city][0].maxWarning==="高危險"){path.classList.add("fill__black")}
